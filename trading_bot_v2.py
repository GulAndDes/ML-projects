...
BASE_CONFIDENCE = 0.58  # Повысили порог входа. Меньше сделок, но качественнее.
...
async def process_market():
    ...
    for sym in active_symbols:
        ...
        try:
            ...
            if adx < 15:
                min_prob = max(base_th, 0.55)  # Во флэте требуем выше вероятность
            elif adx > 30:
                min_prob = max(base_th - 0.03, 0.52)  # В тренде можно чуть ниже
            else:
                min_prob = base_th
